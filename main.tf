...
module "vpc" {
  source = "git::https://github.com/kosalagit/coalfire-vpc.git"
...
module "ec2" {
  source = "git::https://github.com/kosalagit/coalfire-ec2.git"
...
module "asg" {
  source = "git::https://github.com/kosalagit/coalfire-asg.git"
  name = "${local.env}-asg"
  ami = "ami-0b0af3577fe5e3532"
  instance_type = "t2.micro"
...
module "s3" {
    source = "git::https://github.com/kosalagit/coalfire-s3.git"
    bucket = "coalfire-assignment"
    region = "us-east-1"
    lifecycle_rule = [{
        id                                     = "Move to glacier"
        enabled                                = true
...
module "alb" {
  source = "git::https://github.com/kosalagit/coalfire-alb.git"
